...
export function renderAssemblyPanel(state: GameState): string {
  ...
  return `<section class="panel assembly ${state.activeTab === 'assembly' ? '' : 'hidden'}" id="panel-assembly">
      ...
      <div class="assembly-grid">
        <div class="weapon-board-wrap"><div class="weapon-board-header"><h3>선택 무기 슬롯 (5x10)</h3><button id="copy-selected-weapon-slot-state" class="chip-state-copy-btn" type="button" aria-label="슬롯 상태 복사" ${selected ? '' : 'disabled'}>📝</button></div><div id="weapon-board" class="weapon-board" role="grid" aria-label="무기 슬롯 보드"></div><div id="power-summary-bar">${stats ? renderPowerSummary(stats, powerPreview) : '<div class="power-summary"><span class="power-summary-value">무기를 선택하세요.</span></div>'}</div><ul class="slot-hint-list"><li><span class="slot-hint-label">장착</span>모듈을 드래그 후 활성 슬롯에 드롭</li><li><span class="slot-hint-label">해제</span>우클릭 · 휠 클릭 · 보유 모듈 패널로 드래그</li></ul><div id="active-signature" data-sig="${[...active].join(',')}" hidden></div></div>

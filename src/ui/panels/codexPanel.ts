...
function renderPowerTab(state: GameState): string {
  ...
  const entries = state.weapons.map((weapon) => {
    ...
    const pct = Math.min(100, capacity > 0 ? (usage / capacity) * 100 : 0)
    const badge = overloaded
      ? '<span class="codex-power-badge overloaded">과부하</span>'
      : '<span class="codex-power-badge">정상</span>'

    // Tally non-null slots
    const tally = new Map<ModuleType, number>()
    for (const slot of weapon.slots) {
      if (slot != null) tally.set(slot, (tally.get(slot) ?? 0) + 1)
    }
    const moduleItems = [...tally.entries()]
      .map(([t, n]) => `<li>${MODULE_EMOJI[t]} ${MODULE_NAME_KR[t]} ×${n} — ⚡${MODULE_POWER_COST[t]}</li>`)
      .join('')

    return `<div class="codex-power-entry${overloaded ? ' overloaded' : ''}">
  <div class="codex-power-head"><strong>${weapon.type === 'pistol' ? '🔫 권총' : '🪖 소총'} (${weapon.id})</strong><span>⚡ ${usage} / ${capacity}</span>${badge}</div>
  <div class="codex-power-bar-wrap"><div class="codex-power-bar${overloaded ? ' overloaded' : ''}" style="width:${pct}%"></div></div>
  ${moduleItems ? `<ul class="codex-power-modules">${moduleItems}</ul>` : '<p class="hint" style="margin:0;font-size:12px">장착된 칩 없음</p>'}
</div>`
  }).join('')
...
function renderCodexHint(): string {
  ...
  if (selectedCodexSubTab === 'power') return '보유한 무기별 전력 소모 현황을 표시합니다.'

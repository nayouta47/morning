...
import { AXE_MAX_STACK, SHOVEL_MAX_STACK, getAxeCount, getGatherScrapReward, getGatherWoodReward, getShovelCount } from '../../core/rewards.ts'
...
function axeCraftView(state: GameState): ActionGaugeView {
  const runningView = craftViewByRecipe(state, 'axe')
  if (state.craftProgress.axe > 0) return runningView
  if (getAxeCount(state) >= AXE_MAX_STACK) {
    return { phase: 'locked', progress: 1, disabled: true, label: '최대치', timeText: `최대 ${AXE_MAX_STACK}개` }
  }
  return runningView
}
...
export function renderCraftActions(state: GameState): string {
  ...
  const axeView = axeCraftView(state)
  ...
  return `<div class="craft-actions" role="group" aria-label="제작 행동">
      ...
      ${renderGaugeButton('craft-axe', `${getResourceDisplay('axe')} 제작 (${formatCost(getCraftRecipeCost(state, 'axe'))})`, '🪓 도끼 제작', axeView)}
...
export function patchCraftButtons(app: ParentNode, state: GameState): void {
  ...
  patchActionGauge(app, 'craft-axe', axeCraftView(state))
  ...
  patchGaugeTitle(app, 'craft-axe', `${getResourceDisplay('axe')} 제작 (${formatCost(getCraftRecipeCost(state, 'axe'))})`)

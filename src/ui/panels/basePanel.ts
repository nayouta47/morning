...
export function renderBasePanel(state: GameState, actionUI: ActionUI, now = Date.now()): string {
  ...
  const dailySection = `<section class="action-group" aria-label="일과 행동"><h3 class="subheading">일과</h3>${renderGaugeButton('go-to-work', '💵 일 나가기 (+2)', '일 나가기 행동', actionUI.goToWork)}${renderGaugeButton('contact-family', '📞 가족에게 연락하기', '가족에게 연락하기 행동', actionUI.contactFamily)}${state.upgrades.adoptDog ? renderGaugeButton('go-for-walk', `🐕🚶 ${state.dogName ?? '강아지'}와(과) 산책하기`, '산책하기 행동', actionUI.goForWalk) : ''}</section>`

...
import { FLEE_SUCCESS_CHANCE, createEnemyLootTable } from './combat.ts'
...
function processSmeltingElapsed(state: GameState, key: SmeltingProcessKey, elapsedMs: number, storageCap: number): void {
  ...
  if (key === 'meltScrap') {
    ...
    for (let i = 0; i < attempts; i += 1) {
      if (state.resources.scrap < 100 || state.resources.chromium < 3 || state.resources.molybdenum < 1) continue
  ...
  if (key === 'meltIron') {
    let produced = 0
    for (let i = 0; i < attempts; i += 1) {
      if (state.resources.iron < 100 || state.resources.nickel < 8) continue
...
function resolveGatherCompletion(state: GameState, key: 'goToWork' | 'gatherWood' | 'gatherScrap' | 'recoverGuideRobot' | 'takeAndroid' | 'goForWalk' | 'contactFamily' | 'cryoSleep', storageCap: number): void {
  ...
  if (key === 'contactFamily') {
    ...
    if (state.timePassedEventDismissed) {
      ...
      narrate(state, getRandomDogContactFamilyLog(name))
    } else {
  ...
  narrate(state, `사건 — ${EVENT_NAMES.lookAround}`)
}
